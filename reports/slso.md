# COB Alignment Report for SLSO

In the table below, "aligned classes" are classes that have at least one ancestor that is a term in COB.

| Class Set | Number of Classes | Number of Aligned Classes | Alignment % |
| ----- | ----- | ----- | ----- |
| All classes (including imports) | 3948 | 2940 | 74.47% |
| Classes in SLSO namespace | 64 | 61 | 95.31% |

SLSO has 3 unaligned roots. An unaligned root is the highest-level in-namespace term without a COB ancestor. To align SLSO with COB, these terms should be moved under COB terms or added to COB.

The table below contains all unaligned roots in SLSO. The column 'Preferred Root?' indicates whether a term has an `IAO:0000700` annotation marking it as a preferred root in the ontology.

| IRI of Root | Label of Root | Preferred Root? | Number of Subclasses |
| http://purl.obolibrary.org/obo/SLSO_0000044 | photo gallery | No | 0 |
| http://purl.obolibrary.org/obo/SLSO_0000053 | research area | No | 0 |
| http://purl.obolibrary.org/obo/SLSO_0000069 | spaceflight program | No | 0 |
